...
#### In-Memory Signing (Recommended for CI/CD)

Use this approach for CI/CD environments where you don't want to store key files:

```bash
# Export your private key in ASCII-armored format
gpg --armor --export-secret-keys ABCD1234 > private-key.asc

# Convert to base64 for storage in GitHub secrets (removes newlines)
base64 -w 0 < private-key.asc > private-key.base64

# On macOS, use:
# base64 -i private-key.asc -o private-key.base64

# Securely delete the temporary files after storing in secrets
shred -u private-key.asc private-key.base64
```

### 3. Configure Credentials

#### Option A: File-Based Signing (Local Development)
...
#### Option B: In-Memory Signing (CI/CD)

For CI/CD environments, use environment variables with the in-memory approach:
...
2. Add these repository secrets:
   ...
   - `GPG_SIGNING_KEY` - Base64-encoded ASCII-armored private key (see GPG Key Setup above)
