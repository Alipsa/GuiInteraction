...
- 2.2. [x] Guard empty option lists: `promptSelect(String, Collection)` in `gi-common/.../AbstractInOut.groovy` calls `options.iterator().next()` and throws on empty collections; add validation or a clearer exception before delegating.
- 2.3. [x] Add Swing SVG support: `gi-swing/.../InOut.groovy` currently bails with "no support". Use Batik so bundled SVG fixtures can be previewed.
- 2.4. [x] Console password fallback: `gi-console/.../InOut.groovy` returns `null` when `System.console()` is unavailable; provide a masked/non-echo stdin fallback for IDEs/CI.
...
- 4.5 [x] Document and standardise null handling policy across all methods to favour exceptions; some return null on failure, others throw exceptions.
